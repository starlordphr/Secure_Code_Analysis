...
public class CaptureReturnValueofRead extends BugChecker implements MethodInvocationTreeMatcher {
  ...
  private static final Matcher<ExpressionTree> RANDOMGEN_NEXTINT_MATCHER =
      //Matchers.instanceMethod().onDescendantOf("java.io.FileInputStream").withSignature("read()");
      //Matchers.instanceMethod().onDescendantOf("java.io.FileInputStream").named("read");
      //Matchers.instanceMethod().onDescendantOf("java.io.FileInputStream").withSignature("read()");
      Matchers.instanceMethod().onExactClass("java.io.FileInputStream").named("read");
